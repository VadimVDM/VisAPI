...
export class CbbClientService {
  ...
  constructor(
    ...
    this.retryAttempts =
      this.configService.get<number>('cbb.retryAttempts') ?? 3;
  ...
  async findContactByPhone(phone: string): Promise<Contact | null> {
    try {
      ...
      const response = await this.makeRequest<FindContactResponse>(
        'GET',
        CBB_ENDPOINTS.FIND_CONTACT,
        {
          params: {
            field_id: 'phone',
            value: phone,
  ...
  async createContact(contactData: CreateContactDto): Promise<Contact> {
    try {
      ...
      this.logger.error(
        `Failed to create contact for ${contactData.phone}:`,
        error,
      );
      throw error;
    }
  ...
  async sendTextMessage(
    ...
    try {
      ...
      const response = await this.makeRequest<MessageResponse>(
        'POST',
        CBB_ENDPOINTS.SEND_TEXT(contactId),
        {
          data: payload,
        },
      );
      ...
      this.logger.debug(
        `Text message sent to contact ${contactId}:`,
        response.data,
      );
      return response.data;
    } catch (error) {
      this.logger.error(
        `Failed to send text message to contact ${contactId}:`,
        error,
      );
      throw error;
    }
  ...
  async sendFileMessage(
    ...
    try {
      ...
      const response = await this.makeRequest<MessageResponse>(
        'POST',
        CBB_ENDPOINTS.SEND_FILE(contactId),
        {
          data: payload,
        },
      );
      ...
      this.logger.debug(
        `File message sent to contact ${contactId}:`,
        response.data,
      );
      return response.data;
    } catch (error) {
      this.logger.error(
        `Failed to send file message to contact ${contactId}:`,
        error,
      );
      throw error;
    }
  ...
  async sendFlow(contactId: number, flowId: number): Promise<MessageResponse> {
    try {
      ...
      const response = await this.makeRequest<MessageResponse>(
        'POST',
        CBB_ENDPOINTS.SEND_FLOW(contactId, flowId),
      );
      ...
      this.logger.error(
        `Failed to send flow ${flowId} to contact ${contactId}:`,
        error,
      );
      throw error;
    }
  ...
  async getContact(contactId: number): Promise<Contact> {
    try {
      ...
      const response = await this.makeRequest<Contact>(
        'GET',
        `${CBB_ENDPOINTS.CONTACTS}/${contactId}`,
      );
  ...
  async getContactById(id: string): Promise<CBBContact | null> {
    try {
      ...
      const response = await this.makeRequest<CBBContact>(
        'GET',
        `/contacts/${id}`,
      );
      ...
      if (!contact.phone || contact.phone === '') {
        this.logger.debug(
          `Contact ${id} doesn't really exist (empty phone field)`,
        );
        return null;
      }
  ...
  async createContactWithFields(data: CBBContactData): Promise<CBBContact> {
    try {
      this.logger.debug(
        `Creating contact with fields for phone: ${data.phone}`,
      );
      ...
      const contactId =
        response.data?.data?.id || response.data?.id || data.phone;
      ...
      for (const [fieldName, fieldValue] of Object.entries(cufs)) {
        try {
          ...
          await this.makeRequest(
            'POST',
            `/contacts/${id}/custom_fields/${fieldId}`,
            {
          ...
          this.logger.error(
            `Failed to update custom field ${fieldName}:`,
            error,
          );
  ...
  async validateWhatsApp(phone: string): Promise<boolean> {
    try {
      this.logger.debug(
        `Validating WhatsApp for ${phone} - assuming available (CBB is WhatsApp-first)`,
      );
  ...
  private async makeRequest<T>(
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        this.logger.debug(
          `CBB API ${method} ${endpoint} (attempt ${attempt}/${this.retryAttempts})`,
        );
        ...
        if (error && typeof error === 'object' && 'response' in error) {
          ...
          this.logger.warn(
            `CBB API ${method} ${endpoint} failed (attempt ${attempt}): ${status} ${message}`,
          );
        } else {
          this.logger.warn(
            `CBB API ${method} ${endpoint} failed (attempt ${attempt}): ${errorInstance.message}`,
          );
        }
  ...
  async sendWhatsAppTemplate(
    ...
    this.logger.debug(
      `Language: ${languageCode}, Parameters: ${parameters.length}`,
    );
    ...
    try {
      ...
      this.logger.debug(
        `WhatsApp template '${templateName}' sent successfully to contact ${contactId}`,
      );
