...
export class TranslationService {
  private readonly countryTranslations: Record<
    string,
    { hebrew: string; flag: string }
  > = {
  ...
  translateOrderData(data: {
    ...
    const visaType =
      this.visaTypeTranslations[visaTypeKey] ||
    ...
    const processingDays =
      this.processingTimeMap[urgencyKey] || '3-5 ימי עסקים';
  ...
  getCountryDetails(country: string): { hebrew: string; flag: string } {
    ...
    return (
      this.countryTranslations[countryKey] ||
      this.countryTranslations[country?.toLowerCase()] || {
        hebrew: country || 'לא ידוע',
        flag: '🌍',
      }
    );
  }
